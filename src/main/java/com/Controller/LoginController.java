...
public class LoginController {
        ...
        for (int i=0; i<usersList.size(); i++) {
            if (usersList.get(i).getEmail().equals(email) && usersList.get(i).getPassword().equals(password)) {
                ...
                if(user.getRole() == 0) {
                    ...
                    adminController.loadAdminPage(user);
    ...
    public void handleInscriptionButton() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("../Vue/Inscription.fxml"));
            ...
            System.out.println("Erreur lors du chargement de la fenêtre d'inscription");
            e.printStackTrace();
        }
    }
